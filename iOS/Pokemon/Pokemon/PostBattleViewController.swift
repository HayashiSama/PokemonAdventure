...
class PostBattleViewController: UIViewController {
    ...
    @IBOutlet var moveListButtons: [UIButton]!
    var user = User()
    ...
    override func viewDidLoad() {
        ...
        let pok = fetchMyPokemon()
        let moves = pok.moves!

        
        for i in 0..<moveListButtons.count{
            if i < (moves.count){
                moveListButtons[i].setTitle(moves[i], for: .normal)
            }
            else{
                moveListButtons[i].setTitle("", for: .normal)
            }
        }
        ...
        if pokelevelup {
            ...
            DispatchQueue.main.async{
                ...
                let moves = rollNewPokemon(name: self.pokemon!.name!, level: Int(self.pokemon!.level))
                print(moves)
                let pok = fetchMyPokemon()
